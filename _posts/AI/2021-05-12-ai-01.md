...
## 1_ 머신러닝이란?
---
![그림1-인공지능](/assets/img/Blog/ai/1-1.PNG/400x400)
...
![그림3-dataset_formulation](/assets/img/Blog/ai/1-3.PNG/400x200)
...
w와 b를 랜덤하게 선택한 후, $$h(x)$$가 sample값을 넣었을 때 올바르게 정답을 예측해 낼 수 있는지 알아봐야겠죠?

위에서 정의한 dataset을 $$h(x)$$ 에 모두 넣어 결과값을 확인해보겠습니다.
...
위 과정에서 input으로 넣을 sample과 $$h(x)$$에서 정의한 parameter는 vector, matrix로 표현됩니다. 따라서 **선수학습으로 "Linear Algebra(선형대수)"** 가 필요합니다.
